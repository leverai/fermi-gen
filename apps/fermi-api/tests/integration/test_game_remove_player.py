...
def test_remove_player_in_lobby_deletes_player_doc(
    api_client: TestClient,
    ...
    """Host removing a player in lobby deletes their player entry."""
    host_headers = get_api_auth_headers(
        'dev.user+rm-lobby-host@example.com',
        'password123',
        'RmLobbyHost',
    )
    ...
    # Create joiner and join
    creds = create_emulator_user_and_get_token(
        'dev.user+rm-lobby-joiner@example.com',
        'password123',
        'RmLobbyJoiner',
    )
    ...
    assert doc['full'] is False


def test_remove_player_after_start_sets_inactive(
    api_client: TestClient,
    get_api_auth_headers: Callable[[str, str, str], dict[str, str]],
    create_private_game: Callable[[dict[str, str]], str],
    get_firestore_doc: Callable[[str], dict[str, Any]],
    ...
    """Host removing a player after start flags them inactive."""
    host_headers = get_api_auth_headers(
        'dev.user+rm-start-host@example.com',
        'password123',
        'RmStartHost',
    )
    game_id = create_private_game(host_headers)
    ...
    creds = create_emulator_user_and_get_token(
        'dev.user+rm-start-joiner@example.com',
        'password123',
        'RmStartJoiner',
    )
    joiner_uid = creds['localId']
    ...
    assert rr.status_code == 200
    doc = get_firestore_doc(game_id)
    assert doc['players'][joiner_uid]['is_active'] is False
    assert doc['full'] is False
...
def test_remove_inactive_or_nonexistent_returns_404(
    api_client: TestClient,
    get_api_auth_headers: Callable[[str, str, str], dict[str, str]],
    create_private_game: Callable[[dict[str, str]], str],
    get_firestore_doc: Callable[[str], dict[str, Any]],
    ...
    """Removing inactive/non-existent player returns 404."""
    host_headers = get_api_auth_headers(
        'dev.user+rm-404-host@example.com',
        'password123',
        'Rm404Host',
    )
    game_id = create_private_game(host_headers)
    _ = _wait_ready(get_firestore_doc, game_id)
    creds = create_emulator_user_and_get_token(
        'dev.user+rm-404-joiner@example.com',
        'password123',
        'Rm404Joiner',
    )
    joiner_uid = creds['localId']
    ...
    # Second removal → 404 (already inactive)
    rr2 = api_client.post(
        '/api/v1/game/remove_player',
        json={'resource_id': game_id, 'player_id': joiner_uid},
        headers=host_headers,
...
def test_player_self_remove_after_start_allowed(
    api_client: TestClient,
    get_api_auth_headers: Callable[[str, str, str], dict[str, str]],
    create_private_game: Callable[[dict[str, str]], str],
    get_firestore_doc: Callable[[str], dict[str, Any]],
    ...
    """Player removing self after start should succeed and mark them inactive."""
    host_headers = get_api_auth_headers(
        'dev.user+rm-self-start-host@example.com',
        'password123',
        'RmSelfStartHost',
    )
    game_id = create_private_game(host_headers)
    ...
    creds = create_emulator_user_and_get_token(
        'dev.user+rm-self-start-joiner@example.com',
        'password123',
        'RmSelfStartJoiner',
    )
    joiner_uid = creds['localId']
    ...
    # Self-remove by joiner during question
    rr = api_client.post(
        '/api/v1/game/remove_player',
        json={'resource_id': game_id, 'player_id': joiner_uid},
        headers=joiner_headers,
