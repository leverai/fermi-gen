...
class TestGetDqDateForUtc:
    ...
    def test_before_2am_returns_previous_day(self) -> None:
    ...
    def test_at_2am_exactly_returns_same_day(self) -> None:
    ...
    def test_afternoon_returns_same_day(self) -> None:
...
class TestGetWindowForDateUtc:
    ...
    def test_window_starts_at_noon_utc(self) -> None:
        date = datetime.date(2024, 12, 18)
        start, end = get_window_for_date_utc(date)

        expected_start = datetime.datetime(  # noqa: DTZ001
            2024, 12, 18, DQ_WINDOW_OPEN_HOUR, 0, 0
        )
        assert start == expected_start

    def test_window_ends_at_2am_next_day(self) -> None:
        date = datetime.date(2024, 12, 18)
        start, end = get_window_for_date_utc(date)

        expected_end = datetime.datetime(  # noqa: DTZ001
            2024, 12, 19, DQ_DAY_START_HOUR, 0, 0
        )
        assert end == expected_end

    def test_window_duration_is_14_hours(self) -> None:
        date = datetime.date(2024, 12, 18)
        start, end = get_window_for_date_utc(date)
...
class TestGetAnswerDeadline:
    ...
    def test_returns_timeout_when_before_window_end(self) -> None:
    ...
    def test_returns_window_end_when_timeout_exceeds_window(self) -> None:
...
class TestIsWithinAdGrace:
    ...
    def test_before_deadline_returns_true(self) -> None:
    ...
    def test_at_deadline_returns_true(self) -> None:
        deadline = datetime.datetime(2024, 12, 18, 14, 0, 30)  # noqa: DTZ001
        now = deadline
        assert is_within_ad_grace(now, deadline) is True
    ...
    def test_past_grace_period_returns_false(self) -> None:
        deadline = datetime.datetime(2024, 12, 18, 14, 0, 30)  # noqa: DTZ001
...
class TestIsWithinQdGrace:
    ...
    def test_within_qd_grace_returns_true(self) -> None:
    ...
    def test_past_qd_grace_returns_false(self) -> None:
        qd = datetime.datetime(2024, 12, 19, 2, 0, 0)  # noqa: DTZ001
...
class TestSecondsUntil:
    ...
    def test_returns_positive_when_target_in_future(self) -> None:
    ...
    def test_returns_negative_when_target_in_past(self) -> None:
