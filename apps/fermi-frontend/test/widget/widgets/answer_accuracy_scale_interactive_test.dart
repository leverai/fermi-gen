...
void main() {
  group('AnswerAccuracyScale - Interactive Input', () {
    ...
    testWidgets('should snap to discrete values immediately',
        (WidgetTester tester) async {
      // Arrange
      const initialAnswer =
          AnswerValue(number: 1, orderOfMagnitude: '', unit: 'm');
      ...
      // Tap at position that should give log 3 (1K)
      // log 3 is at 3/18 = 16.67% of drawWidth
      const padding = 12.0;
      final drawWidth = width - (padding * 2);
      final targetX = padding + (3 / 18.0) * drawWidth;

      await tester.tapAt(Offset(targetX, size.height / 2));
      await tester.pump();

      // Assert - should get discrete value 1K
      expect(capturedAnswers.length, greaterThan(0));
      final answer = capturedAnswers.last;
      expect(answer.number, 1);
      expect(answer.orderOfMagnitude, 'K');
    });
    ...
    testWidgets('should clamp to valid range (0-18)',
        (WidgetTester tester) async {
      // Arrange
      const initialAnswer =
          AnswerValue(number: 1, orderOfMagnitude: '', unit: 'm');
      ...
      // Tap at extreme left (before padding)
      await tester.tapAt(Offset(0, size.height / 2));
      await tester.pump();
      ...
      // Tap at extreme right (after padding)
      await tester.tapAt(Offset(size.width, size.height / 2));
      await tester.pump();

      // Assert - should clamp to maximum (999 Qa)
      expect(capturedAnswer, isNotNull);
      expect(capturedAnswer!.orderOfMagnitude, 'Qa');
    });

    testWidgets('should produce discrete values only',
        (WidgetTester tester) async {
      // Arrange
      const initialAnswer =
          AnswerValue(number: 1, orderOfMagnitude: '', unit: 'm');
      ...
      // Drag across the entire scale
      const padding = 12.0;
      final startX = padding;
      final endX = width - padding;

      // Simulate multiple taps at different positions
      for (int i = 0; i < 10; i++) {
        final x = padding + (i / 9.0) * (endX - startX);
        await tester.tapAt(Offset(x, size.height / 2));
        await tester.pump();
      }

      // Assert - all values should be discrete
      for (final answer in capturedAnswers) {
        // Number should be 1, 10, or 100
        expect([1, 10, 100], contains(answer.number));
        // OM should be valid
        expect(
            ['', 'K', 'M', 'B', 'T', 'Qa'], contains(answer.orderOfMagnitude));
      }
    });
