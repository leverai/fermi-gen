...
class QuestionScreenV2Controller extends ChangeNotifier {
    ...
    _gameSub = realtime.watchGame(gameId).listen(
      (snapshot) {
        ...
        final bool isTransitionToAnswering = _isTransitionToAnsweringPhase(
          previous: _previousGameState,
          current: snapshot.state,
        );
        if (isTransitionToAnswering) {
          _clearAllRoundScores();
        }
        _previousGameState = snapshot.state;
  ...
  bool _isTransitionToAnsweringPhase({
    ...
    final bool wasFinished = previous == GameState.questionNFinished ||
        previous == GameState.questionLastFinished;
    final bool isAnswering =
        current == GameState.questionN || current == GameState.questionLast;

    return wasFinished && isAnswering;
  }

  /// Clear round scores for all players (hides transient score chips)
  void _clearAllRoundScores() {
    for (final controller in _playerManager.playerControllers.values) {
