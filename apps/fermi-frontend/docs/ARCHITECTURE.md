...
Invite URLs use HTTPS format for maximum compatibility:
...
| Platform | Behavior |
|----------|----------|
| **Web Browser** | Opens web app directly, parses URL path |
| **Android (app installed)** | App Links intercept URL, opens native app |
| **iOS (app installed)** | Universal Links intercept URL, opens native app |
| **Mobile (no app)** | Opens web app, can show app store prompt |
...
- Game service generates invite URLs using `WEB_BASE_URL` config
- Daily Question service generates invite URLs using `WEB_BASE_URL` config
- URLs point to web domain, not API endpoints

**Native App Configuration**:

- **Android**: `AndroidManifest.xml` includes App Links intent filters with `android:autoVerify="true"`
- **iOS**: `Runner.entitlements` includes Associated Domains capability
- Verification files served at `/.well-known/` paths for domain verification

### Flow

1. User shares invite link (generated by backend)
2. Recipient clicks link
3. Platform determines if native app is installed
4. If installed: Native app opens via App Links/Universal Links
5. If not installed: Web app opens in browser
